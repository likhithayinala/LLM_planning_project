...
data_path = '/batches/' #./safety-data/'
eval_ds = CustomDataset(data_path)
eval_loader = DataLoader(eval_ds, batch_size = 1)

model_name = "PKU-Alignment/beaver-dam-7b" # "./checkpoint-37571" # "./qwen2.5_1.5B_judge"
tokenizer = AutoTokenizer.from_pretrained("PKU-Alignment/beaver-dam-7b", pad_token='<pad>', return_tensors='pt')
...
for batch in tqdm(eval_loader):
    ...
    for data in batch[0]:        
        ...
        res.append(output.logits.detach().numpy())
        new_dataset.append((prompt, torch.argmax(output.logits.detach())))
        print()
